...
public:
    ...
    std::string operator()(const libzcash::SaplingPaymentAddress& zaddr) const
    {
        ...
        return bech32::Encode(m_params.Bech32HRP(CChainParams::EGGLING_PAYMENT_ADDRESS), data);
    }
...
public:
    ...
    std::string operator()(const libzcash::SaplingExtendedSpendingKey& zkey) const
    {
        ...
        std::string ret = bech32::Encode(m_params.Bech32HRP(CChainParams::EGGLING_EXTENDED_SPEND_KEY), data);
...
namespace KeyIO {
    ...
    libzcash::PaymentAddress DecodePaymentAddress(const std::string& str)
    {
        ...
        if (bech.first == Params().Bech32HRP(CChainParams::EGGLING_PAYMENT_ADDRESS) &&
    ...
    libzcash::SpendingKey DecodeSpendingKey(const std::string& str)
    {
        std::vector<unsigned char> data;
        auto bech = bech32::Decode(str);
        if (bech.first == Params().Bech32HRP(CChainParams::EGGLING_EXTENDED_SPEND_KEY) &&
